...
def clean_dataset17(file_path):

    cleaned_file_path = "cleaned_dataset_17.csv"

    if not os.path.exists(cleaned_file_path):
        # Perform data cleaning steps if the cleaned dataset doesn't exist
        df = pd.read_csv(r"C:\Users\iremo\PycharmProjects\pythonProject1\ocd_patient_dataset-17.csv")
    ...
    X = df.drop(columns=['Anxiety Diagnosis'])
    y = df['Anxiety Diagnosis']
