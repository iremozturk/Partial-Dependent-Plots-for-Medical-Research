...
def clean_dataset5(file_path):

    cleaned_file_path = "cleaned_dataset_5.csv"

    if not os.path.exists(cleaned_file_path):
        # Perform data cleaning steps if the cleaned dataset doesn't exist
        df = pd.read_csv(
            "C:\\Users\\iremo\\PycharmProjects\\pythonProject1\\healthy-places-index-hpi-diagnosis-group-by-setting-and-patient-category.csv")
        # Drop the "id" column
    ...
    X = df.drop(columns=['HPI Percentile Ranking'])
    y = df['HPI Percentile Ranking']
